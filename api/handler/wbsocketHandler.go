...
func WsConnectHandler(c echo.Context) error {
	...
	if tokenString == "" || roomname == "" {
		return c.JSON(400, "token or roomName is empty")
	}

	token, err := services.DecodeJWTToken(tokenString);
	if err != nil {
		log.Println("DecodeJWTToken error:", err);
...
func WsDisconnectHandler(c echo.Context) error {
	tokenString := c.QueryParam("token");
	if tokenString == "" {
		return c.JSON(400, "token is empty")
	}
	token, err := services.DecodeJWTToken(tokenString);
	if err != nil {
		return c.JSON(500, err.Error())
